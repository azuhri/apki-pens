<div class="table-responsive p-0">
    <table class="table align-items-center mb-0">
        <thead>
            <tr>
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Nama Denah
                </th>
        ...
        <tbody>
            ...
            @foreach ($datas as $data)
                <tr>
                    <td>{{ $data->name }}</td>
                    <td>
                        <a target="_blank" href="{{ url('/') . "/{$data->path}" }}">
                            <img style="height: 150px" src="{{ asset($data->path) }}" alt="">
                        </a>
                    </td>
                    <td class="text-center">{{$data->updateBy->name ?? ""}}</td>
                    <td class="">
                        <div class="d-flex justify-content-center">
                            <a href="javascript:void(0);" onclick="deleteMap({{$data->id}})"
