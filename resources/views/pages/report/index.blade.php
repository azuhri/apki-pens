                                    ...
                                    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                                        ...
                                        <div class="btn-group" role="group">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                <li onclick="sorting('DESC');"><a class="dropdown-item" href="#">Terbaru ke terlama</a></li>
                                                <li onclick="sorting('ASC');"><a class="dropdown-item" href="#">Terlama ke terbaru</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                <select onchange="changeStatus(this);" class="form-select my-2" aria-label="Default select example">
...
@push('js')
    <script>
        ...
        let search, orderBy, sortBy, status;

        const getData = async () => {
            let loading = skeleton(2);
            $("#containerData").html(loading)
            await fetch(`{{ route('user.my-report.list') }}?order_by=${orderBy ?? ''}&sort_by=${sortBy ?? ''}&status=${status ?? ''}&search=${search ?? ''}`)
                .then(response => {
                    if (!response.ok) {
        ...
        const getDetailData = async (id) => {
            ...
            await fetch(`{{ route('user.my-report.detail', '') }}/` + id)
                .then(response => {
                    if (!response.ok) {
                        throw new Error(`HTTP error! Status: ${response.status}`);
                    }
